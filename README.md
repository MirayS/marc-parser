...
`CodeList\Relators` and `CodeList\Fields` (every field and subfield of MARC 21 Bibliographic
through Update No. 34 (July 2022), with its label and repeatability, including the embedded
holdings block) are generated by `tools/generate-codelists.php` from id.loc.gov and the
supplement in `tools/data`. `tools/generate-marc8-tables.php` generates the MARC-8 code tables
the same way. `Validation\Validator`
